...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  2 09:52:03 2020
...
class RussianExerciserCreator:
    ...
    def preposition_substitution(self, pr = ["на", "в", "от", "с", "к", "со", "ко", "про", "до"]):
            ...
            for r in range(len(verbs_list)):
                if phrases_listed[i][0].lower() in verbs_list[r]:
                    for rr in range(2, len(sns)):
                        if verbs_list[r][0].lower() == sns.loc[rr, 3] or verbs_list[r][0].lower() == sns.loc[rr, 4]:
                            aaa = ['________ (' + sns.loc[rr, 3] +' / ' + sns.loc[rr, 4] +')']
                            break
                        elif rr == len(sns) -1:
                            aaa = ['________ (' + verbs_list[r][0] +')']
                            break
                ...
                for k in range(len(verbs_list)):
                    ...
                    if len(phrases_listed[i][j]) > 0:
                        if phrases_listed[i][j][-1] in [',', '.', '!', '?']:
                            if phrases_listed[i][j][:-1].lower() in verbs_list[k]:
                                for rr in range(2, len(sns)):
                                    if verbs_list[k][0].lower() == sns.loc[rr, 3] or verbs_list[k][0].lower() == sns.loc[rr, 4]:
                                        if phrases_listed[i][j-1].lower() in fu:
                                            aaa[-1] = '________ (' + sns.loc[rr, 3] + ' / ' + sns.loc[rr, 4] +')' + phrases_listed[i][j][-1]
                                            break
                                        else:
                                            aaa.append('________ (' + sns.loc[rr, 3] + ' / ' + sns.loc[rr, 4] +')' + phrases_listed[i][j][-1])
                                            break
                            ...
                            if phrases_listed[i][j].lower() in verbs_list[k]:
                                for rr in range(2, len(sns)):
                                    if verbs_list[k][0].lower() == sns.loc[rr, 3] or verbs_list[k][0].lower() == sns.loc[rr, 4]:
                                        if phrases_listed[i][j-1].lower() in fu:
                                            aaa[-1] = '________ (' + sns.loc[rr, 3] + ' / ' + sns.loc[rr, 4] +')'
                                            break
                                        
                                        else:
                                            aaa.append('________ (' + sns.loc[rr, 3] + ' / ' + sns.loc[rr, 4] +')')
                                            break
...
if methods == '4':
    ...
    root4 = tk.Tk()
    root4.withdraw()
    verbs = filedialog.askopenfilename()
    exercise = RussianExerciserCreator(file_path)
